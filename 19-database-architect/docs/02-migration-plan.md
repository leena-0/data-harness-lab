...
- [v] `seeds/sample-data.sql`로 정상 데이터 입력
- [v] 음수 가격, 0개 수량, 범위를 벗어난 평점 거부 확인
- [v] 중복 이메일과 외부 거래 식별자 거부 확인
- [v] 동일 주문의 성공 결제 두 건 거부 확인
- [v] 사용자 삭제 후 `orders.user_id`가 NULL로 변경되는지 확인
- [v] 거래 기록이 존재하는 주문과 상품의 삭제가 거부되는지 확인
- [v] UPDATE 후 `updated_at`이 자동 변경되는지 확인
